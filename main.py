...
class EasyApplyLinkedin:
    ...
    def __init__(self, data):
        ...
        self.driver = self.driver = webdriver.Chrome(data['driver_path'])
    ...
    def job_search(self):
        ...
        search_keywords = self.driver.find_element("css selector", ".jobs-search-box__text-input")
        search_keywords.clear()
        search_keywords.send_keys(self.keywords)
        search_location = self.driver.find_element("css selector", ".jobs-search-box__text-input[aria-label='City, state, or zip code']")
    ...
    def filter(self):
        ...
        all_filters_button = self.driver.find_element("css selector", ".jobs-search-box__text-input")
        all_filters_button.submit()
        time.sleep(1)
        easy_apply_button = self.driver.find_element('xpath', "//label[@for='f_LF-f_AL']")
        easy_apply_button.click()
        time.sleep(1)
        apply_filter_button = self.driver.find_element('xpath', "//button[@data-control-name='all_filters_apply']")
    ...
    def find_offers(self):
        ...
        if total_results_int > 24:
            ...
            get_last_page = self.driver.find_element('xpath', "//button[@aria-label='Page "+str(total_pages_int)+"']")
    ...
    def submit_apply(self,job_add):
        ...
        try:
            in_apply = self.driver.find_element('xpath', "//button[@data-control-name='jobdetails_topcard_inapply']")
            in_apply.click()
        ...
        try:
            submit = self.driver.find_element('xpath', "//button[@data-control-name='submit_unify']")
        ...
        # ... if not available, discard application and go to next
            ...
            try:
                discard = self.driver.find_element('xpath', "//button[@data-test-modal-close-btn]")
                discard.send_keys(Keys.RETURN)
                time.sleep(1)
                discard_confirm = self.driver.find_element('xpath', "//button[@data-test-dialog-primary-btn]")
